...
def file_match_iter(dataroot):
    pattern = re.compile(r'(?P<file>.*(?P<fold>[a-zA-Z0-9_]+)\\'r'(?P<class>hem|all)\\'r'UID_(?P<subject>H?\d+)_(?P<image>\d+)_(?P<cell>\d+)_(all|hem).bmp)')
...
TF_VALID_ROT = transforms.Compose([
    transforms.CenterCrop(300),
    transforms.Resize((224,224), Image.LANCZOS),  #1
    transforms.Lambda(tf_rotation_stack),
])
