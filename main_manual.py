...
def train_validate(args):
    ...
    valid_loader = DataLoader(validset, batch_size=args.batch_size, num_workers=0, shuffle=False)
...
def train_test(args):
    ...
    trainset, class_weights = get_dataset(args.dataroot, folds_train=(0, 1, 2, 3), folds_valid=None)
    class_weights = class_weights.to(args.device)
    print(f"Trainset length: {len(trainset)}")
    print(f"class_weights = {class_weights}")
    ...
    for e in trange(args.epochs, desc='Epoch'):
        ...
        scheduler.step(e)#swap with train_loss
    torch.save(model.state_dict(), f'{args.out}/model.pt')
...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default=24) #16 #max33
    parser.add_argument('--epochs', type=int, default=8) #6
